...
\figref{fig:discussion3:a3}, \figref{fig:discussion3:a05}はそれぞれ, 学習時の速度と比較して, 入力速度が低速倍率$a=3.0$と高速倍率$a=0.5$の場合のSNNの内部状態変化を示す.
横軸は時間 (s) を表す.
...
低速倍率 (\figref{fig:discussion3:a3:snn}) の場合, 時間経過に従って, SNN内部状態の時間変化が小さくなることがわかる.
また, 高速倍率 (\figref{fig:discussion3:a05:snn}) の場合, タイムスケールが変化したのにも関わらず, 内部状態の周期は$a=1.0$のときからの変化が小さい.
...
目標速度が$a=3.0$の低速 (\figref{fig:discussion3:a3:proposed}) のとき, 提案手法のSNNにおける内部状態の周期は, $a=1.0$から3.0倍程度に変化している.
また, 目標速度が$a=0.5$の高速 (\figref{fig:discussion3:a05:proposed}) のとき, 提案手法のSNNにおける内部状態の周期は, $a=1.0$から0.5倍程度に変化している.
...
\begin{figure}[htbp]  %画像形式はこのチャプターのreadmeを参照
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \subcaption{通常のSNN, $a=3.0$}
        \label{fig:discussion3:a3:snn}
    \end{minipage}
    \hspace{0.02\textwidth}
    \begin{minipage}{0.45\textwidth}
        \centering
        \includesvg[width=1.0\textwidth, inkscapelatex=false]{dummy/dummy_img}
        \subcaption{提案手法, $a=3.0$}
        \label{fig:discussion3:a3:proposed}
    \end{minipage}

    \label{fig:discussion3:a3}
    \caption[軌道予測におけるSNN最終層の内部状態変化 (低速倍率変化)]{
        軌道予測におけるSNN最終層の内部状態変化 (低速倍率変化)
        (a) 通常のSNN
        (b) 提案手法
    }
\end{figure}
...
\begin{figure}[htbp]  %画像形式はこのチャプターのreadmeを参照
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includesvg[width=1.0\textwidth, inkscapelatex=false]{dummy/dummy_img}
        \subcaption{通常のSNN, $a=0.5$}
        \label{fig:discussion3:a05:snn}
    \end{minipage}
    \hspace{0.02\textwidth}
    \begin{minipage}{0.45\textwidth}
        \centering
        \includesvg[width=1.0\textwidth, inkscapelatex=false]{dummy/dummy_img}
        \subcaption{提案手法, $a=0.5$}
        \label{fig:discussion3:a05:proposed}
    \end{minipage}

    \label{fig:discussion3:a05}
    \caption[軌道予測におけるSNN最終層の内部状態変化 (高速倍率変化)]{
        軌道予測におけるSNN最終層の内部状態変化 (高速倍率変化)
        (a) 通常のSNN
        (b) 提案手法
    }
\end{figure}
