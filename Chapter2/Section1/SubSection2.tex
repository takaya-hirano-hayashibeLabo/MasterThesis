...
本研究では, 誤差逆伝播法をSNNの学習に拡張したSpatio-Temporal Backpropagation (STBP)則を用いる.
...
STBPにおける時間的・空間的な損失勾配の概要を\figref{fig:snn:stbp}に示す. 
\figref{fig:snn:stbp}の (A) は単一のニューロン, (B) はネットワークにおける勾配の伝播を表す.
...
それぞれの導関数の外形図を\figref{fig:snn:stbp:g}に示す.
...
\begin{figure}[htbp]
    \centering
    % ↓これはる↓
    % https://www.frontiersin.org/journals/neuroscience/articles/10.3389/fnins.2018.00331/full#:~:text=of%20spike%20activity.-,Thus%2C,in%20Equation%20(11)%E2%80%93(21)%20for%20STBP%20can%20be%20approximated%20by%3A,-%E2%88%82
    \includesvg[width=1.0\textwidth, inkscapelatex=false]{dummy/dummy_img}
    \caption{スパイク出力関数$g(v)$とその導関数の近似関数}
