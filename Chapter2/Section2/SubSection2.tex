...
そのため, このときの入力スパイク$o(t)$と内部状態$v(t)$を表すと\eqrefc{eq:actual:state}となる.
\begin{align}
...
さらに, \eqrefc{eq:actual:state:laplace}における$s$を$s/a$に置き換えると, \eqrefc{eq:actual:state:laplace2}得られる.
\begin{align}
